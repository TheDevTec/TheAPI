...
public class v1_18_R1 implements NmsProvider {
	...
	public Object createAnvilContainer(Inventory inv, Player player) {
		ContainerAnvil container = new ContainerAnvil(((CraftPlayer) player).getHandle().nextContainerCounter(),
				...
				ContainerAccess.a(((CraftPlayer) player).getHandle().t, v1_18_R1.zero));
		for (int i = 0; i < 2; ++i)
	...
	public Object packetRespawn(Player player) {
		...
		return new PacketPlayOutRespawn(worldserver.q_(), worldserver.aa(), BiomeManager.a(worldserver.E()),
				entityPlayer.d.b(), entityPlayer.d.c(), worldserver.ad(), worldserver.D(), true);
	}
