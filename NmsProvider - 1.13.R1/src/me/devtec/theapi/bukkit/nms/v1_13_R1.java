...
public class v1_13_R1 implements NmsProvider {
	...
	public Object packetPlayerListHeaderFooter(String header, String footer) {
		...
		packet.a=(IChatBaseComponent)toIChatBaseComponent(ComponentAPI.fromString(header));
		packet.b=(IChatBaseComponent)toIChatBaseComponent(ComponentAPI.fromString(footer));
		return packet;
	}
	...
	public Object createAnvilContainer(Inventory inv, Player player) {
		...
		for(int i = 0; i < 2; ++i)
			container.setItem(i, (net.minecraft.server.v1_13_R1.ItemStack) asNMSItem(inv.getItem(i)));
		return container;
	}
	...
	public void loadParticles() {
		for(MinecraftKey s : Particle.REGISTRY.keySet())
			me.devtec.theapi.bukkit.game.particles.Particle.identifier.put(s.getKey(), Particle.REGISTRY.get(s));
	}
