...
public class Reader {
	private static TypeAdapter<Collection<Object>> adapterList = new Gson().getAdapter(new TypeToken<Collection<Object>>(){});
	private static TypeAdapter<Map<Object,Object>> adapterMap = new Gson().getAdapter(new TypeToken<Map<Object,Object>>(){});
	...
	private static Object parse(Class<?> clazz, Object object) {
		if(clazz==Ref.nms("IChatBaseComponent")||clazz==Ref.nms("IChatMutableComponent")||clazz==Ref.nms("ChatBaseComponent")||clazz==Ref.nms("ChatMessage")
					||clazz==Ref.nms("ChatComponentText")||clazz==Ref.getClass("net.md_5.bungee.api.chat.TextComponent"))
	...
	private static Object parseR(Object o) {
		if(o instanceof Collection) {
			...
			for(Object s : cloneOfList)((Collection<Object>)o).add(parseR(s));
			return o;
		}
		if(o instanceof Map) {
			...
			for(Entry<?, ?> s : ((Map<?,?>)o).entrySet()) {
				if(s.getKey().toString().startsWith("enum ")) {
					a.put(s.getKey(), Ref.getNulled(Ref.getClass((s.getKey().toString()).replaceFirst("enum ", "")), s.getValue()+""));
					c=true;
				}else
					if(s.getKey().toString().startsWith("class ")) {
						a.put(s.getKey(), parse(Ref.getClass((s.getKey().toString()).replaceFirst("class ", "")), parseR(s.getValue())));
						c=true;
					}else
						if(s.getKey().toString().startsWith("Map ")) {
							Map map = (Map)Ref.newInstance(Ref.constructor(Ref.getClass((s.getKey().toString()).replaceFirst("Map ", ""))));
							for(Entry<?,?> er : ((Map<?,?>)s.getValue()).entrySet())
								map.put(parseR(er.getKey()), parseR(er.getValue()));
							a.put(s.getKey(), parseR(map));
							c=true;
						}else
							if(s.getKey().toString().startsWith("Multimap ")) {
								Multimap map = (Multimap)Ref.newInstance(Ref.constructor(Ref.getClass((s.getKey().toString()).replaceFirst("Multimap ", ""))));
								for(Entry<?,?> er : ((Multimap<?,?>)s.getValue()).entries())
									map.put(parseR(er.getKey()), parseR(er.getValue()));
								a.put(s.getKey(), parseR(map));
								c=true;
							}else
							if(s.getKey().toString().startsWith("Collection ")) {
								Collection map = (Collection)Ref.newInstance(Ref.constructor(Ref.getClass((s.getKey().toString()).replaceFirst("Collection ", ""))));
			...
			if(a.size()==1 && c)
				for(Object oa : a.values())return oa;
			return a;
		}
		return o;
	}
	...
	private static Object fixObject(Object o) {
		if(o instanceof Collection)return fix((Collection<?>) o);
		if(o instanceof Map)return fix((Map<?,?>) o);
		if(o instanceof Multimap)return fix((Multimap<?,?>) o);
		return parseR(o);
	}
	...
	private static Object fix(Map<?,?> col){
		...
		if(fix.size()==1)
			for(Object o : fix.values())return o;
		return fix;
	}
	...
	private static Object fixUknown(Object o) {
		if(o instanceof Map) {
			...
			if(preFix instanceof Map<?,?> == false)return preFix;
			else {
		...
		if(o instanceof Multimap) {
			...
			if(preFix instanceof Multimap<?,?> == false)return preFix;
			else {
		...
		if(o instanceof Collection) {
			...
			if(preFix instanceof Collection<?> == false)return preFix;
			else {
