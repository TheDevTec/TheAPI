...
public class NMSAPI {
	...
	private Constructor<?> pDestroy, pTitle, pOutChat, pTab, pBlock, blockPos, pChunk, ChunkSection, chunkc, particle,
			pSpawn, pNSpawn, pLSpawn, pWindow;
	private Method getmat, getb, getc, gett, WorldHandle, PlayerHandle, sendPacket, ichatcon, getser, plist, block,
	...
	public NMSAPI() {
		...
		try {
			...
			try {
				pWindow = Reflections.getNMSClass("PacketPlayOutOpenWindow").getConstructor(int.class, Containers,
						ichat);
			} catch (Exception e3) {
				e3.printStackTrace();
			}
		...
		try {
			pSpawn = Reflections.getNMSClass("PacketPlayOutSpawnEntity")
					.getConstructor(Reflections.getNMSClass("Entity"), int.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			pNSpawn = Reflections.getNMSClass("PacketPlayOutNamedEntitySpawn")
					.getConstructor(Reflections.getNMSClass("EntityHuman"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			pLSpawn = Reflections.getNMSClass("PacketPlayOutSpawnEntityLiving")
					.getConstructor(Reflections.getNMSClass("EntityLiving"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
		...
		try {
			itemstack = Reflections.getBukkitClass("inventory.CraftItemStack").getMethod("asNMSCopy",
		...
		try {
			setblock = Reflections.getNMSClass("DataPaletteBlock").getMethod("setBlock", int.class, int.class,
					int.class, Object.class);
		} catch (Exception e) {

			try {
				setblock = Reflections.getNMSClass("DataPaletteBlock").getMethod("setBlock", int.class, int.class,
						int.class, iblockdata);
			} catch (Exception e2) {
				e2.printStackTrace();
			}
		}
		try {
			setblockb = Reflections.getNMSClass("DataPaletteBlock").getMethod("b", int.class, int.class, int.class,
					Object.class);
		} catch (Exception e) {
		}
		try {
		...
		try {
			pTitle = Reflections.getNMSClass("PacketPlayOutTitle").getConstructor(enumTitle, ichat, int.class,
					int.class, int.class);
		} catch (Exception e) {
		...
		try {
			sendPacket = Reflections.getNMSClass("PlayerConnection").getMethod("sendPacket",
					Reflections.getNMSClass("Packet"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			pBlock = Reflections.getNMSClass("PacketPlayOutBlockChange")
					.getConstructor(Reflections.getNMSClass("IBlockAccess"), pos);
		} catch (Exception ea) {
			try {
				pBlock = Reflections.getNMSClass("PacketPlayOutBlockChange")
						.getConstructor(Reflections.getNMSClass("World"), pos);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		try {
			pChunk = Reflections.getNMSClass("PacketPlayOutMapChunk").getConstructor(Reflections.getNMSClass("Chunk"),
					int.class);
		} catch (Exception e) {
			try {
				pChunk = Reflections.getNMSClass("PacketPlayOutMapChunk")
						.getConstructor(Reflections.getNMSClass("Chunk"), boolean.class, int.class);
			} catch (Exception e1) {
				// packet not exists
			}
		}
	...
	public Object getPacketPlayOutTitle(TitleAction action, Object IChatBaseComponent, int fadeIn, int stay,
			int fadeOut) {
		try {
			return pTitle.newInstance(enumTitle.getField(action.name()).get(null), IChatBaseComponent, fadeIn, stay,
					fadeOut);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public int[] setBlock(World world, int x, int y, int z, Material material, int data, boolean applyPsychics) {
		...
		try {
			...
			worldset.invoke(xx, f, b, applyPsychics ? 3 : 2);
		} catch (Exception e) {
			e.printStackTrace();
		}
