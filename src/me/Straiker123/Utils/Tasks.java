...
public class Tasks {
	...
	private static boolean con(Entity s) {
		...
		if (s.getType() != EntityType.PLAYER) {
			for (String d : LoaderClass.config.getConfig().getStringList("Options.LagChecker.ChunkMobLimit.Bypass"))
				...
				} else if (d.equalsIgnoreCase(s.getType().name())) {
					c = true;
					break;
				}
		}
	...
	public static void load() {
		...
		if (load)
			return;
		load = true;
		if (TheAPI.isOlder1_9() && !TheAPI.getServerVersion().equals("v1_7_R4")
				&& !TheAPI.getServerVersion().startsWith("v1_8"))
			s.add(new Tasker() {
				@Override
				public void run() {
					TheAPI.msg("&bTheAPI&7: &8********************", TheAPI.getConsole());
					TheAPI.msg("&bTheAPI&7: &6Info: &cYour server version isn't supported! ("
							+ TheAPI.getServerVersion() + ")", TheAPI.getConsole());
					TheAPI.msg("&bTheAPI&7: &8********************", TheAPI.getConsole());
				}
		...
		if (LoaderClass.config.getConfig().getBoolean("Options.LagChecker.Enabled"))
			s.add(new Tasker() {
				...
				@Override
				public void run() {
					if (Bukkit.getWorlds().size() <= next)
						next = 0;
					if (TheAPI.getMemoryAPI().getFreeMemory(true) <= c
							.getDouble("Options.LagChecker.ClearMemIfPercentIsFree")) {
						synchronized (this) {
							String sd = TheAPI.getMemoryAPI().clearMemory();
							if (c.getBoolean("Options.Options.LagChecker.Log"))
								TheAPI.msg("&f[&bTheAPI - LagChecker&f] Cleared " + sd + " memory",
										TheAPI.getConsole());
						}
					}
					if (c.getBoolean("Options.LagChecker.ChunkMobLimit.Use")) {
						...
						for (Chunk d : Bukkit.getWorlds().get(next).getLoadedChunks()) {
							if (d.getEntities().length < c.getInt("Options.LagChecker.ChunkMobLimit.Limit"))
								continue;
							...
							if (es.isEmpty() == false)
								ent.put(d.getBlock(0, 0, 0).getLocation(), es);
						}
						for (Location loc : ent.keySet()) {
							if (ent.get(loc).size() < c.getInt("Options.LagChecker.ChunkMobLimit.Limit"))
								continue;
							if (c.getString("Options.LagChecker.ChunkMobLimit.OnLimitExceeded").equalsIgnoreCase("kill")
									|| c.getString("Options.LagChecker.ChunkMobLimit.OnLimitExceeded")
											.equalsIgnoreCase("remove")) {
								if (c.getBoolean("Options.LagChecker.Log"))
									TheAPI.msg("&f[&bTheAPI - LagChecker&f] Killed (" + ent.size() + ") entities",
											TheAPI.getConsole());
								for (Entity e : ent.get(loc)) {
									e.remove();
								}
							} else
								TheAPI.msg("&f[&bTheAPI - LagChecker&f] Too many entities (" + ent.size()
										+ ") in chunk X:" + loc.getBlockX() + ", Z:" + loc.getBlockZ()
										+ " in the world " + loc.getWorld().getName(), TheAPI.getConsole());
