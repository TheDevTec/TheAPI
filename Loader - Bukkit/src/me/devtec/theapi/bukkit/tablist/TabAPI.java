...
public class TabAPI {
	public static void setTabListName(Player p, String name) {
		...
		Ref.set(((List<?>) Ref.get(obj, "b")).get(0), "d", BukkitLoader.getNmsProvider().toIChatBaseComponent(ComponentAPI.fromString(name == null ? p.getName() : name)));
		BukkitLoader.getPacketHandler().send(BukkitLoader.getNmsProvider().getOnlinePlayers(), obj);
	}
