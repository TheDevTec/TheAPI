...
public class BukkitCommandHolder {
	static final Map<Integer, String> EMPTY_MAP = Collections.unmodifiableMap(new HashMap<>());
	static final BukkitPermissionsChecker DEFAULT_PERMISSIONS_CHECKER = (player, perm) -> {return player.hasPermission(perm);};
	
	private CommandStructure[] args;
	private BukkitPermissionsChecker check = DEFAULT_PERMISSIONS_CHECKER;
	
	public BukkitCommandHolder(CommandStructure[] structure) {
	...
	public static BukkitCommandHolder create(CommandStructure... structure) {
		return new BukkitCommandHolder(structure);
	}
	...
	public boolean process(CommandSender sender, String[] s) {
			...
			for(CommandStructure str : args) {
				if(str instanceof EmptyCommandStructure) {
					if(str.getPermission()!=null && !check.check(sender, str.getPermission()))
						return true;
					((BukkitCommandTask)str.getCommandArguments().get(-1)).process(sender, EMPTY_MAP);
					return true;
				}
			}
			return true;
		...
		for(CommandStructure str : args) {
			...
			if(r!=null) {
				...
				if(r!=null)((BukkitCommandTask)r).process(sender, buildSelectors);
			}
			return true;
		}
		return false;
	...
	public BukkitCommandHolder permsChecker(BukkitPermissionsChecker object) {
	...
	public BukkitCommandHolder addCommandStructure(CommandStructure subcmd) {
		args=Arrays.copyOf(args, args.length+1);
		args[args.length-1]=subcmd;
		return this;
	}
	
	public BukkitCommandHolder register(Plugin plugin, String name, String... aliases) {
