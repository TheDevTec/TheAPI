...
public class EventManager {
	...
	public static ListenerHolder register(EventListener listener) {
		return register(0, listener);
	}
	
	public static ListenerHolder register(int priority, EventListener listener) {
		ListenerHolder e = new ListenerHolder();
		e.listener=listener;
		List<ListenerHolder> list = map.get(priority);
		if(list==null)map.put(priority, list=new ArrayList<>());
		list.add(e);
		return e;
	}
	...
	public static void unregister(ListenerHolder handler) {
		...
		for(Entry<Integer, List<ListenerHolder>> entry : map.entrySet()) {
			if(entry.getValue().contains(handler)) {
				entry.getValue().remove(handler);
				if(entry.getValue().isEmpty())removeId.add(entry.getKey());
			}
			
		}
		for(Integer i : removeId)map.remove(i);
	}

	public static void call(Event event) {
		for(ComparableObject<Integer, List<ListenerHolder>> cache : SortingAPI.sortByKeyArray(map, false))
