...
public class v1_14_R1 implements NmsProvider {
	...
	public Object getNBT(ItemStack itemStack) {
		return ((net.minecraft.server.v1_14_R1.ItemStack) this.asNMSItem(itemStack)).getOrCreateTag();
	}
	...
	public ItemStack setNBT(ItemStack stack, Object nbt) {
		...
		net.minecraft.server.v1_14_R1.ItemStack i = (net.minecraft.server.v1_14_R1.ItemStack) this.asNMSItem(stack);
	...
	public Object packetSetSlot(int container, int slot, int stateId, Object itemStack) {
		return new PacketPlayOutSetSlot(container, slot,
				(net.minecraft.server.v1_14_R1.ItemStack) (itemStack == null ? this.asNMSItem(null) : itemStack));
	}
	...
	public Object packetBlockChange(World world, Position position) {
		...
		try {
			v1_14_R1.pos.set(packet, position.getBlockPosition());
		} catch (Exception e) {
		}
		return packet;
	}
	...
	public Object packetScoreboardScore(Action action, String player, String line, int score) {
		return new PacketPlayOutScoreboardScore((ScoreboardServer.Action) this.getScoreboardAction(action), player,
				line, score);
	}
	...
	public void setGUITitle(Player player, Object container, String legacy, int size, String title) {
		...
		NonNullList<net.minecraft.server.v1_14_R1.ItemStack> nmsItems = ((Container) container).items;
		BukkitLoader.getPacketHandler().send(player, this.packetOpenWindow(id, legacy, size, title));
		int i = 0;
		for (net.minecraft.server.v1_14_R1.ItemStack o : nmsItems)
	...
	public void openGUI(Player player, Object container, String legacy, int size, String title, ItemStack[] items) {
		...
		for (int i = 0; i < items.length; ++i) {
			...
			((Container) container).setItem(i, item = (net.minecraft.server.v1_14_R1.ItemStack) this.asNMSItem(is));
			nmsItems[i] = item;
		}
		BukkitLoader.getPacketHandler().send(player, this.packetOpenWindow(id, legacy, size, title));
		int i = 0;
	...
	public void openAnvilGUI(Player player, Object con, String title, ItemStack[] items) {
		...
		for (int i = 0; i < items.length; ++i) {
			...
			if (is == null || is.getType() == Material.AIR)
				continue;
			net.minecraft.server.v1_14_R1.ItemStack item = null;
			container.setItem(i, item = (net.minecraft.server.v1_14_R1.ItemStack) this.asNMSItem(is));
			nmsItems[i] = item;
		}
	...
	public Object createAnvilContainer(Inventory inv, Player player) {
		...
		for (int i = 0; i < 2; ++i)
			container.setItem(i, (net.minecraft.server.v1_14_R1.ItemStack) this.asNMSItem(inv.getItem(i)));
		return container;
	}
	...
	public boolean processInvClickPacket(Player player, HolderGUI gui, Object provPacket) {
		...
		net.minecraft.server.v1_14_R1.InventoryClickType nmsType = packet.g();
		InventoryClickType type = InventoryClickType.valueOf(nmsType.name());

		Object container = gui.getContainer(player);
		ItemStack item = this.asBukkitItem(packet.f());
		if ((type == InventoryClickType.QUICK_MOVE || type == InventoryClickType.CLONE
				...
				&& (item.getType() == Material.AIR || item.getType() == Material.CAVE_AIR))
	...
	public Object packetEntityHeadRotation(Entity entity) {
		return new PacketPlayOutEntityHeadRotation((net.minecraft.server.v1_14_R1.Entity) this.getEntity(entity),
	...
	public Object packetRespawn(Player player) {
		...
		return new PacketPlayOutRespawn(worldserver.worldProvider.getDimensionManager(),
				entityPlayer.world.getWorldData().getType(), entityPlayer.playerInteractManager.getGameMode());
	}
