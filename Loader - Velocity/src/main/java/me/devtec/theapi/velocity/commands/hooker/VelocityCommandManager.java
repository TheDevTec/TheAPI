    ...
    public void register(CommandHolder<?> commandHolder, String command, String[] aliases) {
        SimpleCommand cmd = new SimpleCommand() {
            ...
            public boolean hasPermission(Invocation invocation) {
                return commandHolder.getStructure().getSenderClass().isAssignableFrom(invocation.source().getClass()) && (commandHolder.getStructure().getPermission() == null || invocation.source().hasPermission(commandHolder.getStructure().getPermission()));
            }
        };
