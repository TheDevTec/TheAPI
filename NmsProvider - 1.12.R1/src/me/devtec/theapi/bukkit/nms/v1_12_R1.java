...
public class v1_12_R1 implements NmsProvider {
	...
	public Object getNBT(ItemStack itemStack) {
		return ((net.minecraft.server.v1_12_R1.ItemStack) this.asNMSItem(itemStack)).getTag();
	}
	...
	public ItemStack setNBT(ItemStack stack, Object nbt) {
		...
		net.minecraft.server.v1_12_R1.ItemStack i = (net.minecraft.server.v1_12_R1.ItemStack) this.asNMSItem(stack);
	...
	public Object packetSetSlot(int container, int slot, int stateId, Object itemStack) {
		return new PacketPlayOutSetSlot(container, slot,
				(net.minecraft.server.v1_12_R1.ItemStack) (itemStack == null ? this.asNMSItem(null) : itemStack));
	}
	...
	public Object packetBlockChange(World world, Position position) {
		...
		try {
			v1_12_R1.pos.set(packet, position.getBlockPosition());
		} catch (Exception e) {
		}
		return packet;
	}
	...
	public Object getChunk(World world, int x, int z) {
		...
		if (loaded == null)
			try {
				...
				chunk = ((IChunkLoader) Ref.get(sworld.getChunkProvider(), this.chunkLoader)).a(sworld, x, z);
				if (chunk != null) {
		...
		if (loaded == null) { // generate new chunk
			...
			if ((IChunkLoader) Ref.get(sworld.getChunkProvider(), this.chunkLoader) instanceof ChunkRegionLoader)
				loader = (ChunkRegionLoader) Ref.get(sworld.getChunkProvider(), this.chunkLoader);
	...
	public void setGUITitle(Player player, Object container, String legacy, int size, String title) {
		...
		NonNullList<net.minecraft.server.v1_12_R1.ItemStack> nmsItems = ((Container) container).a();
	...
	public void openGUI(Player player, Object container, String legacy, int size, String title, ItemStack[] items) {
		...
		for (int i = 0; i < items.length; ++i) {
			...
			((Container) container).setItem(i, item = (net.minecraft.server.v1_12_R1.ItemStack) this.asNMSItem(is));
			nmsItems[i] = item;
		}
		BukkitLoader.getPacketHandler().send(player, this.packetOpenWindow(id, legacy, size, title));
		int i = 0;
	...
	public void openAnvilGUI(Player player, Object con, String title, ItemStack[] items) {
		...
		for (int i = 0; i < items.length; ++i) {
			...
			if (is == null || is.getType() == Material.AIR)
				continue;
			net.minecraft.server.v1_12_R1.ItemStack item = null;
			container.setItem(i, item = (net.minecraft.server.v1_12_R1.ItemStack) this.asNMSItem(is));
			nmsItems[i] = item;
		}
	...
	public Object createAnvilContainer(Inventory inv, Player player) {
		ContainerAnvil container = new ContainerAnvil(((CraftPlayer) player).getHandle().inventory,
				((CraftPlayer) player).getHandle().world, v1_12_R1.zero, ((CraftPlayer) player).getHandle());
		for (int i = 0; i < 2; ++i)
			container.setItem(i, (net.minecraft.server.v1_12_R1.ItemStack) this.asNMSItem(inv.getItem(i)));
		return container;
	}
	...
	public Object packetEntityHeadRotation(Entity entity) {
		return new PacketPlayOutEntityHeadRotation((net.minecraft.server.v1_12_R1.Entity) this.getEntity(entity),
