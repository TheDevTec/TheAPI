...
public class ResourcePackAPI {
	...
	public static void setResourcePack(Player player, String resourcePack, String sha, boolean requireRP,
			@Nullable String prompt, ResourcePackHandler handler) {
		...
		BukkitLoader.getPacketHandler().send(player,
				BukkitLoader.getNmsProvider().packetResourcePackSend(resourcePack, sha, requireRP, prompt));
	}
