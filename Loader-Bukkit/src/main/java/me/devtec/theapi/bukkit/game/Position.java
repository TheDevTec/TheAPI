...
public class Position implements Cloneable {
	private static Map<Long, Object> tempMapWithChunks = new TempMap<>(20 * 60);
	...
	public BlockDataStorage getType() {
		BlockDataStorage storage = BlockDataStorage.fromData(getIBlockData()).setItemData(
				BukkitLoader.getNmsProvider().getData(getNMSChunk(), getBlockX(), getBlockY(), getBlockZ()));
		if (BukkitLoader.getNmsProvider().isTileEntity(getNMSChunk(), getBlockX(), getBlockY(), getBlockZ()))
			storage.setNBT(
					BukkitLoader.getNmsProvider().getNBTOfTile(getNMSChunk(), getBlockX(), getBlockY(), getBlockZ()));
		return storage;
	}
	...
	public Object getNMSChunk() {
		if (cachedChunk == null)
			cachedChunk = tempMapWithChunks.computeIfAbsent(getChunkKey(),
					key -> BukkitLoader.getNmsProvider().getChunk(getWorld(), getBlockX() >> 4, getBlockZ() >> 4));
		return cachedChunk;
	}
	...
	public void setTypeAndUpdate(BlockDataStorage type, boolean updatePhysics) {
		...
		if (type.getNBT() != null
				&& BukkitLoader.getNmsProvider().isTileEntity(getNMSChunk(), getBlockX(), getBlockY(), getBlockZ()))
	...
	public boolean equals(Object a) {
		if (a instanceof Position) {
			...
			return (world == null ? s.getWorldName() == null : world.equals(s.getWorldName())) && s.getX() == x
					&& s.getY() == y && s.getZ() == z && s.getPitch() == pitch && s.getYaw() == yaw;
		}
		if (a instanceof Location) {
			Location s = (Location) a;
			return (world == null ? s.getWorld() == null : world.equals(s.getWorld().getName())) && s.getX() == x
					&& s.getY() == y && s.getZ() == z && s.getPitch() == pitch && s.getYaw() == yaw;
		}
		return false;
	}
	...
	public static void updateBlockAt(Position pos, BlockDataStorage blockData) {
		Object packet = BukkitLoader.getNmsProvider().packetBlockChange(pos, blockData.getIBlockData(),
				blockData.getItemData());
		List<Player> players = new ArrayList<>();
		for (Player player : BukkitLoader.getOnlinePlayers())
			if (player.getWorld().getName().equals(pos.getWorldName()))
				players.add(player);
	...
	public void updatePhysics() {
		BukkitLoader.getNmsProvider().updatePhysics(getNMSChunk(), getBlockX(), getBlockY(), getBlockZ(),
				getIBlockData());
	}
	...
	public int hashCode() {
		...
		hash = 17 * hash + Long.hashCode(Double.doubleToLongBits(x));
		hash = 17 * hash + Long.hashCode(Double.doubleToLongBits(y));
		hash = 17 * hash + Long.hashCode(Double.doubleToLongBits(z));
