...
public class BossBar {
	...
	static {
		if (Ref.isBefore(9, 0)) {
			...
			setLocation = Ref.method(Ref.nms("", "Entity"), "setLocation", double.class, double.class, double.class,
	...
	private void set(String text, double progress) {
		...
		BukkitLoader.getPacketHandler().send(holder,
				BukkitLoader.getNmsProvider().packetEntityMetadata(entityId, setupDataWatcher()));
	}

	private void spawnBar(Location loc) {
		before = loc.getWorld();
		entityBar = Ref.newInstance(BossBar.entityWitcherConstructor,
				BukkitLoader.getNmsProvider().getWorld(loc.getWorld()));
