...
@Plugin(id = "theapi", name = "TheAPI", version = "${project.version}", authors = { "DevTec",
...
public class VelocityLoader {
	...
	public static void initTheAPI(ProxyServer server) {
		...
		API.library = new LibraryLoader() {
			...
			Constructor<?> classLoader = Ref.constructor(Ref.getClass("com.velocitypowered.proxy.plugin.PluginClassLoader"), URL[].class);
			Method addToClassloaders = Ref.method(Ref.getClass("com.velocitypowered.proxy.plugin.PluginClassLoader"), "addToClassloaders");
			...
			public void load(File file) {
				...
				loaded.add(file);
				try {
					Ref.invoke(Ref.newInstance(classLoader, (Object)new URL[] { file.toURI().toURL() }), addToClassloaders);
